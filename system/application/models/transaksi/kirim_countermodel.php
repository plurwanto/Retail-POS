<?php
class Kirim_countermodel extends Model {
    ...
    function getkirimList($num, $offset,$id,$with)
	{
    	...
    	$sql = "
			...
			from
			(
			select * from trans_kirim_counter_header where FlagDelete= 'T' $clause order by NoDokumen desc Limit $offset,$num
			) as h
			left join
			(
    ...
    function num_terima_row($id,$with){
		...
		$sql = "SELECT NoDokumen FROM trans_kirim_counter_header where FlagDelete='T' $clause";
	...
	{
		$sql = "SELECT SumberOrder,NoOrder,TglDokumen from trans_kirim_counter_header where NoDokumen='$no'";
		return $this->getRow($sql);
	}
...
KdSatuanKecil ,(select keterangan from satuan where NamaSatuan=KdSatuanKecil) as NamaKecil 
	...
	{
		$sql = "SELECT NoKirimCounter FROM setup_no where Tahun='$tahun'";
		return $this->getRow($sql);
	}
	...
	function getDetail($id)
	{
		...
		SELECT * from trans_kirim_counter_detail Where NoDokumen='$id' order by PCode
		) d
		left join
		(
	...
	function getHeader($id)
	{
			...
			from
			(
			select * from trans_kirim_counter_header where FlagDelete= 'T' and NoDokumen='$id'
			) as h
			left join
			(
			select KdCounter,Keterangan as Nama from counter
