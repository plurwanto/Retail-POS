...
class report_model extends Model
{
	...
	{
            $sql = "SELECT 
                        d.PCode, h.NoStruk, h.Tanggal, h.KdStore, h.NoKassa, h.Kasir, SUM(d.Qty) AS Qty, d.Harga, SUM(d.Netto) AS Netto, mb.NamaLengkap, h.KdCustomer, h.NamaCustomer 
                      FROM
                        transaksi_header h 
                        INNER JOIN transaksi_detail d 
                          ON h.NoStruk = d.NoStruk 
                        INNER JOIN masterbarang mb 
                          ON d.PCode = mb.PCode 
                       WHERE d.STATUS = '1' 
                       $NoStruk $Tgltrans $kondisi $cs
                      GROUP BY d.PCode";
		/*$sql = "
