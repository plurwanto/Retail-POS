<?php
class Retur_barangmodel extends Model {
	
    function __construct(){
        parent::__construct();
    }

    function getreturList($num, $offset,$id,$with)
	{
	 	if($offset !=''){
			$offset = $offset;
		}            
    	...
    	$sql = "
			select NoDokumen,date_format(TglDokumen,'%d-%m-%Y') as Tanggal,kontak.Nama,retur.Keterangan,CountQty,Bruto
			from
			(
			select * from trans_retur_header where FlagDelete= 'T' $clause order by NoDokumen desc Limit $offset,$num
			) as retur
			left join
			(
			select KdSupplier,NamaSupplier as Nama from supplier
			) kontak
			on kontak.KdSupplier = retur.KdSupplier";
		return $this->getArrayResult($sql);
    }
    
    function num_retur_row($id,$with){
     	$clause="";
     	if($id!=''){
			if($with=="NoDokumen"){
				$clause = "and $with like '%$id%'";
		...
		$sql = "SELECT NoDokumen FROM trans_retur_header where FlagDelete='T' $clause";
	...
	function getKontak(){
    	$sql = "SELECT KdSupplier,Keterangan as Nama from supplier order by KdSupplier";
		return $this->getArrayResult($sql);
    }
	
	function getSumber($no)
	{
		$sql = "SELECT SumberOrder,NoOrder,TglDokumen from trans_retur_header where NoDokumen='$no'";
		return $this->getRow($sql);
	}
...
KdSatuanKecil ,(select keterangan from satuan where NamaSatuan=KdSatuanKecil) as NamaKecil 
	...
	function getKirim($kirim,$perusahaan)
	{
		...
		keterangan as NamaSatuan,KdContact from(
			...
			and d.FlagBalik='Y' and d.Flagretur='T' and h.FlagDelete='T' and d.FlagDelete='T' and month(h.TglDokumen)=(select month(TglTrans) from aplikasi ORDER BY Tahun DESC LIMIT 0,1) and year(h.TglDokumen)=(select year(TglTrans) from aplikasi ORDER BY Tahun DESC LIMIT 0,1)
			order by PCode desc
		)brg
		left join
		(
	...
	function getPCodeDet($kode,$field,$thn)
	{
		$sql = "
                        select PCode,NamaInitial,HargaBeliAkhir,$field from(
                        select KodeBarang,$field from stock where KodeBarang='$kode' and tahun='$thn')a
                        inner join
                        (SELECT PCode,NamaLengkap as NamaInitial,HargaBeliAkhir FROM masterbarang)b
	...
	{
		$sql = "SELECT Noretur FROM setup_no where Tahun='$tahun'";
		return $this->getRow($sql);
	}
	function cekPast($no,$pcode)
	{
		$sql = "select distinct Qtyretur,HargaBeliAkhir
		from trans_retur_detail
	...
	{
		$sql = "
		select d.PCode,QtyRetur,HargaBeliAkhir,NamaInitial from(
		SELECT * from trans_retur_detail Where NoDokumen='$id' and FlagDelete='T' order by PCode
		) d
		left join
		(
	...
	{
		$sql = "
		select d.PCode,Qtyretur,QtyHargaretur,NamaInitial from(
		SELECT * from trans_retur_detail Where NoDokumen='$id' order by PCode
		) d
		left join
		(
			select PCode,NamaLengkap as NamaInitial from masterbarang
	...
	{
		$sql = "SELECT NoDokumen,date_format(TglDokumen,'%d-%m-%Y') as TglDokumen,h.Keterangan,h.keterangan,
		c.NamaSupplier as supplier,h.KdSupplier,c.Alamat1,c.Alamat2,c.Kota,c.Telepon,c.NoFax
		from trans_retur_header h,supplier c
...
//            print $sql;
	...
	{
		$sql = "select FlagDelete from trans_retur_detail where NoDokumen='$id' and PCode='$pcode'";
		return $this->getRow($sql);
	}
