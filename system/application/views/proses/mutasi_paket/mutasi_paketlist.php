...
<script language="javascript">
	...
	function deleteTrans(nodok,url)
	{
		...
		if(r==true){
			$.post(url+"index.php/proses/mutasi_paket/delete_penerimaan",{
				kode:nodok},
			function(data){
//                            alert("data");
				window.location = url+"index.php/proses/mutasi_paket";
			});
		}
	}
	function PopUpPrint(kode,baseurl)
	{
		url="index.php/proses/mutasi_paket/cetak/"+escape(kode);
	...
	<tr>
		...
		<td>
			<select size="1" height="1" name ="searchby" id ="searchby" onchange="gantiSearch()">
				...
				<option value="NoPO">Keterangan</option>
			</select>
		</td>
...
?>
		...
		<?php
			...
			{
		?>
			<img src='<?=base_url();?>public/images/printer.png' border = '0' title = 'Print' onclick="PopUpPrint('<?=$data[$a]['id_mutasi'];?>','<?=base_url();?>');"/></a>
		<?php
			}
		?>
		</td>
<?php } ?>
		<td nowrap><?=$data[$a]['id_mutasi'];?></td>
		<td nowrap><?=$data[$a]['Tanggal'];?></td>
		<td nowrap><?=$data[$a]['MPCode'];?></td>
		<td nowrap><?=stripslashes($data[$a]['NamaLengkap']);?></td>
		<td nowrap><?=$data[$a]['Qty'];?></td>
		<td nowrap><?=stripslashes($data[$a]['Jenis_mutasi']);?></td>
...
?>
	<tr>
	<td nowrap colspan="3">
		<a 	href="<?=base_url();?>index.php/proses/mutasi_paket/add_new/"><img src='<?=base_url();?>public/images/add.png' border = '0' title = 'Add'/></a>
	</td>
<?php } ?>
</table>
<?php
