...
?>
	...
	<tr>
		...
		<td><?=$Satuan[$a]?></td>
		<td>
		<input type='text' maxlength="3" size="3" id='qtycetak<?=$a?>' name='qtycetak[]' value='<?=$Qty[$a];?>'/>
		</td>
	</tr>
...
?>
	<tr><td colspan=5>
	    <?php if($NoDokumen!="110000"&&$NoDokumen!="111000")
		{
		?>
		...
		?>
		<input type='hidden' id="nodokumen" name="nodokumen" value="<?=$NoDokumen?>">
		</td></tr>
    </table>
</table>
...
function SaveData()
	{
			str = "";
			var lastRow = document.getElementsByName("qtycetak[]").length;
			for(index=0;index<lastRow;index++){
				nama = document.getElementsByName("qtycetak[]");
				temp = nama[index].id;
				//alert(temp);
				indexs = temp.substr(4,temp.length-4);
				nilai = $("#qtycetak"+index).val();
				//alert(nilai);
				str += nilai + "~";
			}
			nodokumen = $("#nodokumen").val();
			//alert(nodokumen);
			window.opener.document.getElementById("qtyctk").value = str;
			window.opener.saveThis(nodokumen);
	}
</script>	
