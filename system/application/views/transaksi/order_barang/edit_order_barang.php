...
<form method='post' name="order" id="order" action='<?=base_url();?>index.php/transaksi/order_barang/save_new_order' onsubmit="return false">
	<table align = 'center' >
		<tr>
			...
			<legend class="legendStyle">Edit Terima</legend>
...
//                        print_r($header);
		...
		<tr>
			...
			<table class="table_class_list" id="detail">
				<tr id="baris0">
                                        <td>
                                            <img src="<?=base_url();?>public/images/table_add.png" width="16" height="16" border="0" onClick="AddNew()" id="newrow">
                                        </td>
					<td>KdBarang</td>
					<td>NamaBarang</td>
					<td>Satuan</td>
			...
			<?php
			$modeller = new order_barangmodel();
//                        print_r($detail);
			for($z=0;$z<count($detail);$z++){
			
				$disabl = "";
				detailThis($z+1,$detail[$z]['PCode'],$detail[$z]['NamaInitial'],$detail[$z]['QtyOrder'],$detail[$z]['HargaOrder'],$disabl);
			}
			?>
