...
<script language="javascript">
	...
	function deleteTrans(nodok,url)
	{
		...
		if(r==true){
			$.post(url+"index.php/transaksi/terima_counter/delete_penerimaan",{
				kode:nodok},
			function(data){
//                            alert("data");
				window.location = url+"index.php/transaksi/terima_counter";
			});
		}
	}
	function PopUpPrint(kode,baseurl)
	{
		url="index.php/transaksi/terima_counter/cetak/"+escape(kode);
	...
	<tr>
		...
		<td>
			<select size="1" height="1" name ="searchby" id ="searchby" onchange="gantiSearch()">
				...
				<option value="TglDokumen">TglPenerimaan</option>
				<option value="TglTerima">TglTerima</option>
			</select>
		</td>
...
?>
		...
		<?php
			...
			{
		?>
		<a 	href="<?=base_url();?>index.php/transaksi/terima_counter/edit_kirim/<?=$data[$a]['NoDokumen'];?>"><img src='<?=base_url();?>public/images/pencil.png' border = '0' title = 'Edit'/></a>
		<?php
			}
			if($link->delete=="Y")
			{
...
?>
	<tr>
	<td nowrap colspan="3">
		<a 	href="<?=base_url();?>index.php/transaksi/terima_counter/add_new/"><img src='<?=base_url();?>public/images/add.png' border = '0' title = 'Add'/></a>
	</td>
<?php } ?>
</table>
<?php
