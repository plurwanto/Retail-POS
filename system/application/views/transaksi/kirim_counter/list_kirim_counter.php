...
<script language="javascript">
	...
	function deleteTrans(nodok,url)
	{
		...
		if(r==true){
			$.post(url+"index.php/transaksi/kirim_counter/delete_kirim_counter",{
				kode:nodok},
			function(data){
//                            alert("data");
				window.location = url+"index.php/transaksi/kirim_counter";
			});
		}
	}
	function PopUpPrint(kode,baseurl)
	{
		url="index.php/transaksi/kirim_counter/cetak/"+escape(kode);
...
?>
		...
		<?php
			...
			{
		?>
		<a 	href="<?=base_url();?>index.php/transaksi/kirim_counter/edit_kirim/<?=$data[$a]['NoDokumen'];?>"><img src='<?=base_url();?>public/images/pencil.png' border = '0' title = 'Edit'/></a>
		<?php
			}
			if($link->delete=="Y")
			{
...
?>
	<tr>
	<td nowrap colspan="3">
		<a 	href="<?=base_url();?>index.php/transaksi/kirim_counter/add_new/"><img src='<?=base_url();?>public/images/add.png' border = '0' title = 'Add'/></a>
	</td>
<?php } ?>
</table>
<?php
