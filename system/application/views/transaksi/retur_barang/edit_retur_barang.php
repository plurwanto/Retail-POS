...
$gantikursor = "onkeydown=\"changeCursor(event,'retur',this)\"";?>
...
<form method='post' name="retur" id="retur" action='<?=base_url();?>index.php/transaksi/retur_barang/save_new_retur' onsubmit="return false">
	<table align = 'center' >
		<tr>
			...
			<legend class="legendStyle">Edit retur</legend>
...
//                        print_r($header);
			...
			echo $mylib->write_textbox("Kontak","kontak",stripslashes($header->supplier),"35","50","readonly='readonly'","text",$gantikursor,"1");
		...
		<tr>
			...
			<?php
			$modeller = new retur_barangmodel();
//                        print_r($detail);
			for($z=0;$z<count($detail);$z++){
			
				$disabl = "";
				detailThis($z+1,$detail[$z]['PCode'],$detail[$z]['NamaInitial'],$detail[$z]['QtyRetur'],$detail[$z]['HargaBeliAkhir'],$disabl);
			}
			?>
		...
		<tr>
			<td nowrap>
				...
				<input type="button" value="Back" ONCLICK =parent.location="<?=base_url();?>index.php/transaksi/retur_barang/" />
			</td>
		</tr>
	</table>
</form>
...
function detailThis($counter,$pcode,$nama,$qty,$hrg,$disabl){ ?>
<tr id="baris<?=$counter?>">
	...
	<td nowrap><input type="text" id="ttl<?=$counter?>" name="ttl[]" size="5" maxlength="11" value="<?=($hrg*$qty)?>" readonly="readonly" ></td>
