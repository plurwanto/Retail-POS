...
<table align = 'center'>
<form method='post' name="subarea" id="subarea" action='<?=base_url();?>index.php/master/subarea/save_subarea'>
	<tr>
		<td nowrap>Kode</td>
		<td nowrap>:</td>
		<td nowrap><input type='text' maxlength="6" size="8" readonly name='kode' id='kode' value='<?=$viewsubarea->KdSubArea;?>' /></td>
	</tr>
	<tr>
		<td nowrap>Nama</td>
		<td nowrap>:</td>
		<td nowrap><input type='text' maxlength="25" size="35" id='nama' name='nama' value='<?=$viewsubarea->Keterangan;?>' <?=$gantikursor;?>/></td>
	</tr>
	<tr>
		<td nowrap>Area</td>
		<td nowrap>:</td>
		...
		for($a = 0;$a<count($master);$a++){
		 	$select = "";
		 	if($viewsubarea->KdArea==$master[$a]['KdArea']){
				$select = "selected";
			}
		?>
