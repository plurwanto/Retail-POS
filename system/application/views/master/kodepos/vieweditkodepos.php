...
<table align = 'center'>
<form method='post' name="kodepos" id="kodepos" action='<?=base_url();?>index.php/master/kodepos/save_kodepos'>
	<tr>
		<td nowrap>Kode Pos</td>
		<td nowrap>:</td>
		<td nowrap><input type='text' readonly maxlength="6" size="8" name='kode' value="<?=$viewkodepos->KodePos;?>" id='kode' <?=$gantikursor;?> /></td>
	</tr>
	<tr>
		<td nowrap>Nama Kode Pos</td>
		<td nowrap>:</td>
		<td nowrap><input type='text' maxlength="40" size="45" id='nama' value="<?=$viewkodepos->Keterangan;?>" name='nama' <?=$gantikursor;?>/></td>
	</tr>
	<tr>
		<td nowrap>Area</td>
		<td nowrap>:</td>
		...
		<?php
			...
			{			 
			?>
			<option <?php if($masterarea[$s]['KdArea']==$viewkodepos->KdArea) echo "selected"; ?> value="<?=$masterarea[$s]['KdArea'];?>"><?=$masterarea[$s]['Keterangan']?></option>
			<?php
			}
			?>
			</select>
