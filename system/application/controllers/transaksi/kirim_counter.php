<?php
class kirim_counter extends Controller {

    function __construct(){
        parent::__construct();
        $this->load->library('globallib');
        $this->load->model('transaksi/kirim_countermodel');
    }
    	...
    	if($sign=="Y")
	{
	    ...
	    $with 		  = $this->input->post('searchby');
	        ...
	        $config['num_links']  	  = 2;
			$config['base_url']       = base_url().'index.php/transaksi/kirim_counter/index/';
			$page					  = $this->uri->segment(4);
			$config['uri_segment']    = 4;
			$flag1					  = "";
			if($with!=""){
                            if($id!=""&&$with!=""){
                                $config['base_url']     = base_url().'index.php/transaksi/kirim_counter/index/'.$with."/".$id."/";
		...
		$data['header']		= array("No Dokumen","Tanggal","Counter","Keterangan");
	        $config['total_rows']	= $this->kirim_countermodel->num_terima_row(addslashes($id),$with);
	        $data['data']	= $this->kirim_countermodel->getkirimList($config['per_page'],$page,addslashes($id),$with);
	        $data['track']  = $mylib->print_track();
			$this->pagination->initialize($config);
	        $this->load->view('transaksi/kirim_counter/list_kirim_counter', $data);
	    }
		else{
    ...
    function add_new(){
    	...
    	if($sign=="Y"){
			$aplikasi = $this->kirim_countermodel->getDate();
//			$data['mperusahaan'] = $this->kirim_countermodel->getPerusahaan();
			$data['aplikasi'] = $aplikasi;
			$data['mcounter'] = $this->kirim_countermodel->getKontak();
	    	$this->load->view('transaksi/kirim_counter/add_kirim_counter',$data);
    	}
		else{
			$this->load->view('denied');
		}
	...
	function carikontak()
	{
		...
		$mkontak = $this->kirim_countermodel->getKontak($with);
		$str = "";
    ...
    function delete_kirim_counter(){
            $mylib = new globallib();
                $id = $this->input->post('kode'); //die();
		$header = $this->kirim_countermodel->getSumber($id);
		$user = $this->session->userdata('userid');
		$tgl2 = $this->session->userdata('Tanggal_Trans');
                $tgl  = $mylib->ubah_tanggal($tgl2);
		$getHeader = $this->kirim_countermodel->getHeader($id);
		$getDetail= $this->kirim_countermodel->getDetailDel($id);
		$tahun = substr($getHeader->TglDokumen,6,4);
		$lastNo = $this->kirim_countermodel->getNewNo($tahun);
		$NoDelete = $id;
		if((int)$lastNo->NoTerima == (int)$NoDelete + 1){
			$this->db->update("setup_no",array("NoTerima"=>$NoDelete[1]),array("Tahun"=>$tahun));
		}
		$this->kirim_countermodel->locktables('trans_kirim_counter_detail,trans_terima_header');
		...
		$fieldkeluar = "QtyKeluar".$bulan;
                ...
                for($s=0;$s<count($getDetail);){
                        ...
                        $stokawal = $this->kirim_countermodel->CekStock($fieldmasuk,$fieldakhir,$pcodebarang,$tahun);
                        $data = array(
		...
		$this->db->delete('trans_kirim_counter_detail',array('NoDokumen' => $id."D"));
		$this->db->delete('trans_terima_header',array('NoDokumen' => $id."D"));
		$this->db->update('trans_kirim_counter_detail',array("FlagDelete" => "Y","DeleteDate"=>$tgl,"DeleteUser"=>$user,"NoDokumen"=>$id."D"), array('NoDokumen' => $id));
		$this->db->update('trans_terima_header',array("FlagDelete" => "Y","DeleteDate"=>$tgl,"DeleteUser"=>$user,"NoDokumen"=>$id."D"), array('NoDokumen' => $id));
		$this->kirim_countermodel->unlocktables();
	...
	function printThis()
	{
		...
		$data['fileName2'] = "kirim_counter.sss";
	...
	function varCetak()
	{
		...
		$id = $this->uri->segment(4);
		$header	 = $this->kirim_countermodel->getHeader($id);
//                print_r($header);
		$data['header']	 = $header;
		$detail	 = $this->kirim_countermodel->getDetailForPrint($id);
//                print_r($detail);
//		$data['pt'] = $this->kirim_countermodel->getAlmPerusahaan($header->KdPerusahaan);
		...
		$tambahan_judul = "";
		$data['judullap'] = "Kirim Barang Counter".$tambahan_judul;
		$data['url'] = "kirim_counter/printThis/".$id;
		$data['colspan_line'] = 4;
...
//			$hasil = $mylib->findSatuanQtyCetak($detail[$m]['QtyTerima'],$detail[$m]['KonversiBesarKecil'],$detail[$m]['KonversiTengahKecil']);
	...
	$sign  = $mylib->getAllowList("edit");
    	if($sign=="Y"){
            $id = $this->uri->segment(4);
            $data['header']	 = $this->kirim_countermodel->getHeader($id);
            $data['detail']	 = $this->kirim_countermodel->getDetail($id);
            $data['mcounter']    = $this->kirim_countermodel->getKontak();
           
            $this->load->view('transaksi/kirim_counter/e_kirim_counter', $data);
    	}else{
            $this->load->view('denied');
	}
    }
    ...
    function getPCode(){
		...
		{
			$valpcode = $this->kirim_countermodel->ifPCodeBarcode($kode);
		}
		if(count($valpcode)!=0)
		{
			$pcode = $valpcode->PCode;
			$detail = $this->kirim_countermodel->getPCodeDet($pcode);
	...
	function getsumber()
	{
		...
		if($order!=""){
			$hasil = $this->kirim_countermodel->getOrder($order,$perusahaan);
		}
		if($kirim!="")
		{
			$hasil = $this->kirim_countermodel->getKirim($kirim,$perusahaan);
		}
		$str = "";
		$strsatuan = "";
		$kendaraan = "";
...
//            die();
		...
		for($x=0;$x<count($pcode1);$x++)
		{
			...
			if($pcode!=""){
				$this->InsertAllDetail($flag,$no,$pcode,$qty,$hrg,$user,$mylib->ubah_tanggal($tgl),$pcodesave,$ket,$kdcounter,$KdGudang);
			}
		}
                /* update sisa order jika order bukan manual*/
              
		redirect('/transaksi/kirim_counter/');
	}
   

        function insertNewHeader($flag,$tgl,$ket,$kdcounter,$user)
	{
		$this->kirim_countermodel->locktables('setup_per_perusahaan,trans_kirim_counter_header');
		$new_no = $this->kirim_countermodel->getNewNo(substr($tgl,0,4));
		$noOK    = $new_no->NoKirimCounter;
                $kd     = split ("-", $noOK);
                $nextNo     =(int)$kd[1];
                $nobaru = $kd[0]."-".($nextNo+1);
		$this->db->update('setup_no', array("NoKirimCounter"=>$nobaru),array("Tahun"=>substr($tgl,0,4)));
		$data = array(
			'NoDokumen'	=> $noOK,
			'TglDokumen'    => $tgl,
			'KdCounter'     => $kdcounter,
		...
		$this->db->insert('trans_kirim_counter_header', $data);
		$this->kirim_countermodel->unlocktables();
		return $noOK;
	}
        ...
        function updateHeader($flag,$no,$ket,$tglterima,$sumber,$noorder,$user,$nopo)
	{
		...
		$this->kirim_countermodel->locktables('trans_terima_header,trans_kirim_counter_detail');
		$data = array(
			'Keterangan'    => $ket ,
			'TglTerima'     => $tglterima,
			'SumberOrder'   => $sumber,
		...
		if($flag=="edit")
		{
			$data['EditDate'] = $tgl;
			$data['EditUser'] = $user;
			$this->db->update('trans_kirim_counter_detail', array('EditDate'=> $tgl,'EditUser'=>$user), array('NoDokumen' => $no));
		}
		$this->db->update('trans_terima_header', $data, array('NoDokumen' => $no));
		$this->kirim_countermodel->unlocktables();
	}

        function InsertAllDetail($flag,$no,$pcode,$qty,$hrg,$user,$tgl,$pcodesave,$ket,$kdcounter,$KdGudang)
	{
//            echo $no."|".$tgl."|PCode=".$pcode."|save".$pcodesave."|qty:".$qty;die();
		if($flag=="add")
		{
                        $this->doAll($flag,$no,$pcode,$qty,$hrg,$user,$tgl,$ket,$pcodesave,$kdcounter,$KdGudang);
		}
		else
		{
			...
			{
//                    echo "ok";die();
				$cekdulu = $this->kirim_countermodel->cekPast($no,$pcode);
...
//                                echo $no."|".$tgl."|PCode=".$pcode."|save".$pcodesave."|".$cekdulu->QtyTerima;
				...
				else
				{
					if($flag=="edit")
					{
						...
						$this->db->update("trans_kirim_counter_detail",$data,array("NoDokumen"=>$no));
					}
				}
			}
			else
			{
				$cekdulu = $this->kirim_countermodel->cekPast($no,$pcodesave);
//                                print_r($cekdulu);die();
                                //$pcodebarang_dulu = $this->kirim_countermodel->ifPCodeBarcode($pcodesave);
        ...
        function doAll($flag,$no,$pcode,$qty,$hrg,$user,$tgl,$ket,$pcodesave,$kdcounter,$KdGudang)
	{
		$bulan = substr($tgl,5,2);
		$tahun = substr($tgl,0,4);
		$fieldmasuk = "QtyMasuk".$bulan;
		...
		$this->insertMutasi($no,$tgl,$pcode,$ket,$qty,$hrg,$user,$kdcounter,$KdGudang);
	}
	function updateStok($pcodebarang,$tahun,$qtyterima,$fieldmasuk,$fieldkeluar,$fieldakhir)
	{
		if($qtyterima!=0){
                        $stokawal = $this->kirim_countermodel->CekStock($fieldkeluar,$fieldakhir,$pcodebarang,$tahun);
        ...
        function insertMutasi($no,$tgl,$pcode,$ket,$qty,$hrg,$user,$kdcounter,$KdGudang)
	{
		if($qty!=0){
				...
				$dataekonomis = array(
					'KdTransaksi'   => "KC",//Kirim Ke Counter
					'NoTransaksi'   => $no,
					'Tanggal'       => $tgl,
                                        'Gudang'        => $KdGudang,
                                        'GudangTujuan'  => $kdcounter,
					'KodeBarang'    => $pcode,
					'Qty'           => abs($qty) ,
					'Nilai'         => abs($hrg) ,
					'Jenis'         => $jenismutasi ,
		...
		if($qtyterima!=0){
                ...
                if($flag!="del"){
                    if($pcode!=$pcodebarang){
                        $this->db->delete("trans_kirim_counter_detail",array("NoDokumen"=>$no,"PCode"=>$pcodebarang));
                    }else{
                        $this->db->delete("trans_kirim_counter_detail",array("NoDokumen"=>$no,"PCode"=>$pcode));
                    }
                }
	}
        function insertDetail($flag,$no,$pcode,$qty,$hrg,$user,$tgl)
	{
		...
		$tgltrans = $this->session->userdata('Tanggal_Trans');
		$this->kirim_countermodel->locktables('trans_kirim_counter_detail');
               ...
               $data = array(
                ...
                $this->db->insert('trans_kirim_counter_detail', $data);
                if($flag=="edit"){
			$data = array(
                                'EditDate'      => $tgltrans,
				'EditUser'      => $user
			);
                    $this->db->update('trans_kirim_counter_detail', $data, array('NoDokumen' => $no,'PCode'=>$pcode));
		}
