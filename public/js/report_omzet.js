...
function ubahkontak(sumber)
{
	...
	$.post(base_url+"index.php/transaksi/order_barang/carikontak",{ sumber:sumber},
	function(data){
...
function getOrder()
{
	if(cekoption("noorder","Memasukkan No Order")){
			...
			base_url = $("#baseurl").val();
			$.post(base_url+"index.php/transaksi/order_barang/getsumber",{ order:noorderan,kirim:""},
			function(data){
				if(data=="+")
				{
...
function getKirim()
{
	if(cekoption("nokirim","Memasukkan No Pengiriman")){
			nokirim = $("#nokirim").val();
			base_url = $("#baseurl").val();
			$.post(base_url+"index.php/transaksi/order_barang/getsumber",{ order:"",kirim:nokirim},
			function(data){
				if(data=="^&&^+")
				{
...
function findPCode(id)
{
	if(cekheader())
	{
		if(cekoption("pcode"+id,"Memasukkan Kode Barang")){
			base_url = $("#baseurl").val();
			pcode = $("#pcode"+id).val();
			$.post(base_url+"index.php/transaksi/order_barang/getRealPCode",{ pcode:pcode},
			function(datakode){
				if(datakode!=""){
					...
					if(!dobel){
						...
						$.post(base_url+"index.php/transaksi/order_barang/getPCode",{ pcode:pcode},
						function(data){
							if(data!="")
							{
								result = data.split("*&^%");
