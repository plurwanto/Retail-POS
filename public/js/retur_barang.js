function loading()
{
	...
	$('#tglretur').datepicker({ dateFormat: 'dd-mm-yy',mandatory: true,showOn: "both", buttonImage: base_url+ "public/images/calendar.png", buttonImageOnly: true } );
}
...
{
	base_url = $("#baseurl").val();
	$.post(base_url+"index.php/transaksi/retur_barang/carikontak",{ sumber:sumber},
	function(data){
...
function getOrder()
{
			noorderan = $("#noorder").val();
			base_url = $("#baseurl").val();
			$.post(base_url+"index.php/transaksi/retur_barang/getsumber",{ order:noorderan,kirim:""},
			function(data){
				if(data=="+")
				{
...
{
			base_url = $("#baseurl").val();
			$.post(base_url+"index.php/transaksi/retur_barang/getsumber",{ order:"",kirim:nokirim},
			function(data){
				if(data=="^&&^+")
				{
...
function pickThis(obj)
{
	if(cekheader())
	{
		...
		url = base_url+"index.php/pop/brgretur/index/"+id+"/";
...
function findPCode(id)
{
	...
	{ //alert(id);
		if(cekoption("pcode"+id,"Memasukkan Kode Barang")){
			...
			pcode = $("#pcode"+id).val();
			tgl   = $("#tgl").val();
			$.post(base_url+"index.php/transaksi/retur_barang/getRealPCode",{ pcode:pcode},
			function(datakode){
				if(datakode!=""){
					...
					if(!dobel){
						...
						$.post(base_url+"index.php/transaksi/retur_barang/getPCode",{ pcode:datakode,tgl:tgl},
						function(data){
							if(data!="")
							{
								result = data.split("*&^%");
							...
							{
								alert("Stock Barang / Data Tidak Ditemukan");
								resetRow(id);
								$("#pcode"+id).focus();
							}
						});
