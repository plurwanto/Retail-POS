...
{
	base_url = $("#baseurl").val();
	$.post(base_url+"index.php/transaksi/kirim_counter/carikontak",{ sumber:sumber},
	function(data){
...
function getOrder()
{
	if(cekoption("noorder","Memasukkan No Order")){
			noorderan = $("#noorder").val();
			base_url = $("#baseurl").val();
			$.post(base_url+"index.php/transaksi/kirim_counter/getsumber",{ order:noorderan,kirim:""},
			function(data){
				if(data=="+")
				{
...
function getKirim()
{
	if(cekoption("nokirim","Memasukkan No Pengiriman")){
			nokirim = $("#nokirim").val();
			base_url = $("#baseurl").val();
			$.post(base_url+"index.php/transaksi/kirim_counter/getsumber",{ order:"",kirim:nokirim},
			function(data){
				if(data=="^&&^+")
				{
...
function findPCode(id)
{
	if(cekheader())
	{
		if(cekoption("pcode"+id,"Memasukkan Kode Barang")){
			base_url = $("#baseurl").val();
			pcode = $("#pcode"+id).val();
			$.post(base_url+"index.php/transaksi/kirim_counter/getRealPCode",{ pcode:pcode},
			function(datakode){
				if(datakode!=""){
					...
					if(!dobel){
						...
						$.post(base_url+"index.php/transaksi/kirim_counter/getPCode",{ pcode:pcode},
						function(data){
							if(data!="")
							{
								result = data.split("*&^%");
...
function cekheader()
{
	...
	if(cekoption("counter","Mengisi Counter"))
        return true;
}
...
function deleteItem(no,tgl,pcode,pcodesave,qty)
{
	if($("#transaksi").val()=="no"){
		$("#transaksi").val("yes");
		base_url = $("#baseurl").val();
		$.post(base_url+"index.php/transaksi/kirim_counter/delete_item",{
