...
//	website : www.pengz.com
	...
	var imgDir = "<?=base_url();?>public/images/calendar/";	// directory for images ... e.g. var imgDir="/img/"
	...
	var	crossobj, crossMonthObj, crossYearObj, monthSelected, yearSelected, dateSelected, omonthSelected, oyearSelected, odateSelected, monthConstructed, yearConstructed, intervalID1, intervalID2, timeoutID1, timeoutID2, ctlToPlaceValue, ctlNow, dateFormat, nStartingYear, ctlToPlaceView,dateViewFormat;
    ...
    function hideElement( elmID, overDiv )
    {
      if( ie )
      {
        for( i = 0; i < document.all.tags( elmID ).length; i++ )
        {
          ...
          if( !obj || !obj.offsetParent || (obj && obj.name && obj.name.toUpperCase().indexOf('CRYSTALREPORT')>-1 ))
          {
            continue;
          }
...
/* END CHANGE */
		  ...
		  elementToOver = document.getElementById(overDiv);
		  elementToHide = document.getElementsByTagName(elmID);
		  ////window.status=(elementToHide[0].style.left + " - "+elementToOver.style.left+" - "+element.style.bottom);
	  }
    }
    ...
    */
	...
	function init()	{
		if (!ns4)
		{
			if (!ie && navigator.appName!="Opera") { yearNow += 1900	}
	...
	function hideCalendar()	{
 		crossobj=(dom)?document.getElementById("calendar").style : ie? document.all.calendar : document.calendar; //ubahan
	...
	function constructDate(d,m,y,_dateFormat)
	{
		sTmp = _dateFormat;
		var _internalDate = new Date(y,m,d);

		////window.status=(_internalD/ate+ " - "+_internalDate.getDay());
		var _internalDayname = new Array("Sun","Mon","Tue","Wed","Thu","Fri","Sat");
	...
	function closeCalendar() {
		...
		if (ctlToPlaceView) ctlToPlaceView.value = constructDate(dateSelected,monthSelected,yearSelected,dateViewFormat);
	...
	function popUpMonth() {
		...
		hideElement( 'OBJECT', document.getElementById("selectMonth") );
		hideElement( 'IFRAME', document.getElementById("selectMonth") );
	}
	...
	function constructCalendar () {
		...
		for	( datePointer=1; datePointer<=numDaysInMonth; datePointer++ )
		{
			...
			dateMessage = "onmousemove='//window.status=\""+selectDateMessage.replace("[date]",constructDate(datePointer,monthSelected,yearSelected,dateFormat))+"\"' onmouseout='//window.status=\"\"' ";
	...
	function popUpCalendar(ctl,	ctl2, format,ctl_view, format_view,icallBackFunction) {
		var	leftpos=0;
		var	toppos=0;
		...
		if (bPageLoaded)
		{
			if ( crossobj.visibility ==	"hidden" ) {
				...
				ctlToPlaceView = ctl_view;
				dateFormat=format;
				dateViewFormat=format_view;
				...
				hideElement( 'IFRAME', document.getElementById("calendar") );
		...
		{
			init();
			popUpCalendar(ctl,	ctl2, format,ctl_view, format_view);
		}
	}
