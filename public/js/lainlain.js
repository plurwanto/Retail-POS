...
$(document).on('focus', '.autocomplete_txt', function () {
    $(this).autocomplete({
        source: function (request, response) {
            $.ajax({
                url: base_url + 'index.php/transaksi/lainlain/getBarangByName',
                dataType: "json",
                method: 'post',
                data: {
...
$('#qty_0').on('keypress', function (e) {
    ...
    var url;
    var nodok = $('#nodok').val();
    url = base_url + 'index.php/transaksi/lainlain/ajax_add';
...
function reload_table() {
    var no = $('#nodok').val();
    $.ajax({
        url: base_url + 'index.php/transaksi/lainlain/ajax_list/' + no,
        type: 'post',
        dataType: 'json',
        success: function (response) {
            if (no != "") {
                ...
                for (var i = 0; i < response.length; i++) {
                    x = i + 1;
                    total = response[i].QtyPcs * response[i].Harga;
                    ...
                    html += '<td width="15%" class="text-right" name="harga[]" id="harga' + x + '">' + response[i].Harga + '</td>';
                    html += '<td width="15%" class="text-right" name="qty[]" id="qty' + x + '">' + response[i].QtyPcs + '</td>';
...
function delete_item(rows)
{
    ...
    if (tbl == 1) {
        ...
        {
            $.ajax({
                url: base_url + 'index.php/transaksi/lainlain/ajax_delete/' + no + '/' + kodebarang,
                type: "POST",
                dataType: "JSON",
                success: function (response)
                {
...
$('#lainlain').on('keyup keypress', function (e) {
